...
from world.sim.action_parser import ActionParser, ActionRequest
...
class TestActionRequest:
    ...
    def test_valid_action_request(self) -> None:
        ...
        request = ActionRequest(action="simulation.start", params={"tick_rate": 30})
        assert request.action == "simulation.start"
    ...
    def test_action_request_with_empty_params(self) -> None:
        ...
        request = ActionRequest(action="simulation.stop")
        assert request.action == "simulation.stop"
    ...
    def test_params_must_be_dict(self) -> None:
        ...
        with pytest.raises(ValueError, match="'params' must be a dictionary"):
            ActionParser().parse({"action": "simulation.start", "params": "not_a_dict"})
...
class TestActionParser:
    ...
    def test_parse_valid_request(self) -> None:
        ...
        parser = ActionParser()
        request = parser.parse({"action": "simulation.start", "params": {"tick_rate": 30}})
        assert request.action == "simulation.start"
        assert request.params == {"tick_rate": 30}
    ...
    def test_parse_with_missing_params(self) -> None:
        ...
        parser = ActionParser()
        request = parser.parse({"action": "simulation.stop"})
        assert request.action == "simulation.stop"
        assert request.params == {}
